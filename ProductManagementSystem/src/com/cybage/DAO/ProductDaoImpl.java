...
public class ProductDaoImpl implements ProductDao{
	...
	static {
		...
		try {
			productDisplayAll=connection.prepareStatement("select * from productmanagement");
			productInsert=connection.prepareStatement("Insert into productmanagement (name,price) values (?,?)");
			productDelete=connection.prepareStatement("delete from productmanagement where id=?");
			productGetById=connection.prepareStatement("select * from productmanagement where id=?");
		}catch(Exception e) {
			e.printStackTrace();
		}
	}
